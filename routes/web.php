...
Route::get('/', function () {
    $comics = [
        ...
        [
            "title" => "Batman vs Superman Annual #1",
            ...
            "price" => "$4.99",
            "series" => "Batman vs Superman Annual",
...
Route::get('/comic/{title}', function ($title) {
    ...
    $title = urldecode($title); // Decodifica il titolo

    // Trova il fumetto per titolo
    $comic = collect($comics)->firstWhere('title', $title);

    if (!$comic) {
        abort(404); // Gestisci l'errore se il comic non esiste
    }

    return view('pages.comic', ['comic' => $comic]);
});
